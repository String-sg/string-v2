...
- **App availability and links**
  ...
  - `isIntranetUrl` returns true for the configured intranet URL patterns in the codebase (regex list in `src/lib/app-access.ts`, e.g., hosts matching `intranet.*`, `.internal`, `.corp`, `.local`).
...
- **Profile share copy**
  - Share action uses Clipboard API with a deprecated `execCommand('copy')` fallback and shows toast on success/failure.
