...
- **App availability and links**
  - `getAppAvailability` flags intranet-only URLs; `isIntranetUrl` returns true for domains matching *.moe.edu.sg (e.g., intranet.moe.edu.sg and nested subdomains like portal.intranet.moe.edu.sg) based on the current matching strategy.
...
- **Deep links**
  ...
  - (Planned/verify) Hitting `?pin=<appId>&addToProfile=true` while signed out prompts auth and resumes action after sign-in, preserving the intent post-auth.
